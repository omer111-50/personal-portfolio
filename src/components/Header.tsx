...
export default function Header({ pathname = "/" }: Props) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-primary/10 shadow-sm">
      <div className="relative backdrop-blur-md bg-background/85">
        <BrandBackdrop intensity={0.35} />
        <div className="container max-w-5xl mx-auto px-6 md:px-4 relative z-10">
          <nav className="flex items-center justify-between py-4">
            {/* Logo */}
            <motion.h2
              className="m-0 text-xl font-bold font-heading"
            ...
            >
              <motion.a
                href="/"
                className="group flex items-center gap-2 brand-gradient-text"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
              ...
              {navLinks.map((link, index) => (
                ...
                >
                  <HeaderLink
                    ...
                    pathname={pathname}
                  >
                    <span className="underline-fx">{link.label}</span>
                  </HeaderLink>
                </motion.div>
              ))}
            </motion.div>
